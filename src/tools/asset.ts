...
export const assetTool: ToolDef = categoryTool(
  ...
  "Asset management: list, search, read, CRUD, import meshes/textures, datatables, stringtables.",
  {
    ...
    describe:       bp("Universal agent-readable text summary. Auto-detects type: Blueprint (parent/components/variables/per-graph pseudocode + perf smells), BehaviorTree (blackboard + tree of composites/tasks/decorators/services), Material (params + blend/shading + expression count). Params: assetPath", "describe_asset", (p) => ({ path: p.assetPath ?? p.path })),
    ...
    proxy_overlap_audit:  bp("Read-only StaticMesh proxy QA for distance z-fighting: audits LOD0 source geometry for tolerance-quantized duplicate triangles, classifies same-facing duplicates versus opposite-facing coincident pairs, checks geometric degenerates, reports PixelDepthOffset coverage, and reads built render-buffer triangle/vertex/section counts for lodIndex? (default 0). Params: assetPath and/or directory, lodIndex?, toleranceCm? (default 0.001), recursive? (default true), offset? (directory pagination, default 0), maxResults? (page size, default 200), maxExamples? (default 20). Returns per-mesh risk factors and builtRender* counts, aggregate totals, hasMore, and nextOffset.", "proxy_overlap_audit", (p) => ({ assetPath: p.assetPath, directory: p.directory, lodIndex: p.lodIndex, toleranceCm: p.toleranceCm, recursive: p.recursive, offset: p.offset, maxResults: p.maxResults, maxExamples: p.maxExamples })),
    proxy_overlap_repair: bp("Plan-first StaticMesh proxy repair for same-facing tolerance-coincident triangles. Cyclic orientation keys preserve reverse-facing shell triangles; protected mounts and n-gons are refused/skipped. Params: assetPath and/or directory, sourceLodIndex? (default 0), toleranceCm? (default 0.001), recursive?, offset?, maxResults?, apply? (default false), save? (default true), preservePolygonGroups? (default true), attributeConflictPolicy? ('skip' default for planning; explicit 'keep_first' required to apply). Rebuilds only modified mesh assets; never compiles code or the engine.", "proxy_overlap_repair", (p) => ({ assetPath: p.assetPath, directory: p.directory, sourceLodIndex: p.sourceLodIndex, toleranceCm: p.toleranceCm, recursive: p.recursive, offset: p.offset, maxResults: p.maxResults, apply: p.apply, save: p.save, preservePolygonGroups: p.preservePolygonGroups, attributeConflictPolicy: p.attributeConflictPolicy })),
    ...
    configure_static_mesh: bp("Configure StaticMesh build settings that need a mesh rebuild (generic set_property cannot trigger one): nanite (enable/disable), lodGroup (engine preset chain), lodCount + lodPercentTriangles? (explicit generated LODs, default halving per level), lightmapResolution, lightmapCoordinateIndex, collisionComplexity (default|simpleAndComplex|useSimpleAsComplex|useComplexAsSimple). Rebuilds once covering all changes. Params: assetPath, any of the above, save? (default true)", "configure_static_mesh", (p) => ({ assetPath: p.assetPath, nanite: p.nanite, lodGroup: p.lodGroup, lodCount: p.lodCount, lodPercentTriangles: p.lodPercentTriangles, lightmapResolution: p.lightmapResolution, lightmapCoordinateIndex: p.lightmapCoordinateIndex, collisionComplexity: p.collisionComplexity, save: p.save })),
  ...
  undefined,
  {
    ...
    save: z.boolean().optional().describe("import_texture_batch / configure_static_mesh / proxy_overlap_repair: save modified packages immediately (default true)"),
    nanite: z.boolean().optional().describe("configure_static_mesh: enable/disable Nanite (triggers a mesh rebuild)"),
    lodCount: z.number().optional().describe("configure_static_mesh: number of LODs to generate (1-8); LOD1+ reduce triangles from LOD0"),
    lodPercentTriangles: z.array(z.number()).optional().describe("configure_static_mesh: per-LOD triangle percentage 0-1 (index matches LOD; default halves per level)"),
    lightmapResolution: z.number().optional().describe("configure_static_mesh: lightmap resolution (e.g. 64, 128, 256)"),
    lightmapCoordinateIndex: z.number().optional().describe("configure_static_mesh: UV channel used for lightmaps"),
    collisionComplexity: z.string().optional().describe("configure_static_mesh: default | simpleAndComplex | useSimpleAsComplex | useComplexAsSimple"),
    ...
    lodIndex: z.number().int().nonnegative().optional().describe("proxy_overlap_audit: built render-buffer LOD to inspect (default 0)"),
    sourceLodIndex: z.number().int().nonnegative().optional().describe("proxy_overlap_repair: source MeshDescription LOD to inspect/repair (default 0)"),
    offset: z.number().int().nonnegative().optional().describe("proxy_overlap_audit: zero-based offset into the sorted directory mesh list (default 0)"),
    toleranceCm: z.number().positive().optional().describe("proxy_overlap_audit: position/degeneracy tolerance in centimeters (default 0.001)"),
    maxExamples: z.number().int().positive().optional().describe("proxy_overlap_audit: maximum duplicate groups returned per mesh (default 20)"),
    apply: z.boolean().optional().describe("proxy_overlap_repair: mutate and rebuild meshes; false plans only (default false)"),
    preservePolygonGroups: z.boolean().optional().describe("proxy_overlap_repair: include polygon group/material identity in the oriented key (default true)"),
    attributeConflictPolicy: z.enum(["skip", "keep_first"]).optional().describe("proxy_overlap_repair: safe plan default is skip; explicit keep_first is required to apply flattened proxies"),
    ...
    lodGroup: z.string().optional().describe("Texture LOD group (World, Character, UI, ...) or configure_static_mesh LOD group preset (LevelArchitecture, SmallProp, LargeProp, ...)"),
