...
export const animationTool: ToolDef = categoryTool(
  ...
  "Animation assets, skeletons, montages, blendspaces, anim blueprints, physics assets.",
  {
    ...
    create_control_rig:   bp("Create a Control Rig blueprint. With skeletalMeshPath the rig imports that mesh/skeleton hierarchy (factory names the asset CR_<mesh>); without it an empty rig named <name> is created. Params: name, packagePath?, skeletalMeshPath?, onConflict?", "create_control_rig", (p) => ({ name: p.name, packagePath: p.packagePath, skeletalMeshPath: p.skeletalMeshPath, onConflict: p.onConflict })),
    add_rig_bone:         bp("Add a bone to a Control Rig hierarchy. Params: assetPath, boneName, parentName?, location? [x,y,z]", "add_rig_bone", (p) => ({ assetPath: p.assetPath, boneName: p.boneName, parentName: p.parentName, location: p.location })),
    add_rig_control:      bp("Add an animation control to a Control Rig. Params: assetPath, controlName, controlType? (transform|float|bool|vector|rotator), parentName?", "add_rig_control", (p) => ({ assetPath: p.assetPath, controlName: p.controlName, controlType: p.controlType, parentName: p.parentName })),
    add_rig_node:         bp("Place a rig unit node on the Control Rig VM graph. unitStruct accepts short names (TwoBoneIKSimplePerItem, GetTransform -> RigUnit_*) or /Script/ControlRig.RigUnit_* paths. Returns the node's pins. Params: assetPath, unitStruct, nodeName?, x?, y?", "add_rig_node", (p) => ({ assetPath: p.assetPath, unitStruct: p.unitStruct, nodeName: p.nodeName, x: p.x, y: p.y })),
    connect_rig_pins:     bp("Link Control Rig VM pins: 'NodeName.PinName' -> 'NodeName.PinName' (nested pins: 'Node.Pin.SubPin'). Params: assetPath, fromPin, toPin", "connect_rig_pins", (p) => ({ assetPath: p.assetPath, fromPin: p.fromPin, toPin: p.toPin })),
    set_rig_pin_default:  bp("Set a Control Rig VM pin default from a string (numbers, bone names, or struct text like (X=0,Y=0,Z=10)). Params: assetPath, pinPath, value", "set_rig_pin_default", (p) => ({ assetPath: p.assetPath, pinPath: p.pinPath, value: p.value })),
    read_rig_graph:       bp("Read a Control Rig: VM nodes with pins/defaults, links, and hierarchy elements. Params: assetPath", "read_rig_graph", (p) => ({ assetPath: p.assetPath })),
  ...
  undefined,
  {
    ...
    location: Vec3.optional().describe("add_rig_bone: initial bone location"),
    controlName: z.string().optional().describe("add_rig_control: control element name"),
    controlType: z.string().optional().describe("add_rig_control: transform (default) | float | bool | vector | rotator"),
    parentName: z.string().optional().describe("Control Rig: parent bone/control for the new element"),
    unitStruct: z.string().optional().describe("add_rig_node: RigUnit_* short name or /Script/ControlRig.RigUnit_* path"),
    nodeName: z.string().optional().describe("add_rig_node: explicit VM node name"),
    fromPin: z.string().optional().describe("connect_rig_pins: source pin path 'Node.Pin'"),
    toPin: z.string().optional().describe("connect_rig_pins: target pin path 'Node.Pin'"),
    pinPath: z.string().optional().describe("set_rig_pin_default: pin path 'Node.Pin[.SubPin]'"),
    value: z.string().optional().describe("set_rig_pin_default: default value as text"),
  },
);
