...
export const blueprintTool: ToolDef = categoryTool(
  ...
  "Blueprint reading, authoring, and compilation, including AnimGraph/EventGraph node scripting: add_node, connect_pins, search_node_types, plus variables, functions, graphs, components, interfaces, and event dispatchers.",
  {
    ...
    export_compact_graph: bp("Export a headless, token-efficient, loss-aware Blueprint graph model for Blueprint-to-C++ analysis. Recursively follows user macros/composites/Blueprint functions up to maxDepth=5; preserves graph/node/pin GUIDs, raw names, full pin types/defaults, hidden/orphan safety flags, normalized exec/data edges, function signatures/flags, locals, parent class and derived override metadata. Unknown node classes are explicit in unsupportedNodes; use export_nodes_t3d for exact round-trip mutation. Params: assetPath, graphName?, maxDepth?, includeHiddenPins?, includeDerivedOverrides?, derivedOverrideLoadLimit?, dumpToFile?, outputPath?, inlineResult?", "export_compact_graph", (p) => ({ path: p.assetPath, graphName: p.graphName ?? "EventGraph", maxDepth: p.maxDepth, includeHiddenPins: p.includeHiddenPins, includeDerivedOverrides: p.includeDerivedOverrides, derivedOverrideLoadLimit: p.derivedOverrideLoadLimit, dumpToFile: p.dumpToFile, outputPath: p.outputPath, inlineResult: p.inlineResult })),
    ...
    describe_graph:    bp("Render graph(s) as compact indented pseudocode (entry points, call sequence, branch/sequence sub-blocks, inline data inputs). Much cheaper to read than read_graph JSON; the natural 'read' half of the author_logic loop. Params: assetPath, graphName? (omit = all graphs)", "describe_blueprint_graph", (p) => ({ path: p.assetPath, graphName: p.graphName })),
    lint:              bp("Scan one Blueprint (assetPath) or a folder (pathFilter, recursive) for anti-patterns: EventTick logic, GetAllActorsOfClass-on-Tick, very large graphs. Returns per-blueprint issue lists. Params: assetPath? | pathFilter?, limit?, largeGraphThreshold?", "lint_blueprints", (p) => ({ assetPath: p.assetPath, pathFilter: p.pathFilter, limit: p.limit, largeGraphThreshold: p.largeGraphThreshold })),
    author_logic:      bp("Compile a constrained C++-style block into wired EventGraph nodes in one call. Top-level defs: `event OnBeginPlay(){...}`, `event OnTick(){...}` (DeltaSeconds bound), `event Name(type p){...}` (custom event, params->bindings). Statements: `UClass::Func(args);`, `Target->Func(args);`, `Func(args);`, `Type Name = Call(args);` (bind), `ExistingVar = Expr;` (set member var), `if (Bool){...} else {...}`. Args: number/\"string\"/true/false/nullptr/SelfActor/DeltaSeconds/binding/nested call. NOT yet: loops, switch, cast, timeline, return values (reported in warnings). Returns an inline `report` (nodes, exec/data connections, open exec outputs, unset data inputs) so no read_graph round-trip is needed to verify wiring. Params: assetPath, code", "author_blueprint_logic", (p) => ({ path: p.assetPath, code: p.code })),
    insert_logic:      bp("Compile a body of statements and splice it into the exec chain right after an existing node (afterNode.Then -> new chain -> old downstream). Same statement grammar as author_logic (no event wrapper). Params: assetPath, afterNodeId (guid or name), code, graphName?", "insert_blueprint_logic", (p) => ({ path: p.assetPath, afterNodeId: p.afterNodeId, code: p.code, graphName: p.graphName })),
    undo_last_authored: bp("Delete the nodes created by the most recent author_logic/insert_logic call on this Blueprint (this session). Params: assetPath", "undo_last_authored", (p) => ({ path: p.assetPath })),
    ...
    import_nodes_t3d:  bp("Paste a T3D node blob into a graph (Ctrl+V equivalent) for bulk authoring (#130). Pre-flights node classes and function references against reflection: validateOnly=true dry-runs and returns unknownClasses/unknownFunctions without mutating; a real import surfaces them as warnings and returns an inline `report` (nodes, exec/data connections, open exec outputs, unset data inputs) so no read_graph round-trip is needed. Params: assetPath, graphName?, t3d, posX?, posY?, validateOnly?", "import_nodes_t3d", (p) => ({ path: p.assetPath, graphName: p.graphName ?? "EventGraph", t3d: p.t3d, posX: p.posX, posY: p.posY, validateOnly: p.validateOnly })),
  ...
  undefined,
  {
    ...
    validateOnly: z.boolean().optional().describe("import_nodes_t3d: dry-run the pre-flight (class/function resolution + importability) without mutating the graph"),
    ...
    dumpToFile: z.boolean().optional().describe("read_graph/export_compact_graph: also write JSON to an absolute or Saved-relative file"),
    outputPath: z.string().optional().describe("Absolute or Saved-relative JSON dump path; default path is generated from the asset and graph"),
    maxDepth: z.number().int().min(0).max(5).optional().describe("export_compact_graph: recursively capture referenced user graphs to this depth (default 0)"),
    includeHiddenPins: z.boolean().optional().describe("export_compact_graph: retain hidden pins and their identity/defaults (default true)"),
    includeDerivedOverrides: z.boolean().optional().describe("export_compact_graph: load and inspect child Blueprint function overrides (default false)"),
    derivedOverrideLoadLimit: z.number().int().min(1).max(5000).optional().describe("export_compact_graph: maximum derived Blueprints to load per owning Blueprint (default 128; truncation is explicit)"),
    inlineResult: z.boolean().optional().describe("export_compact_graph: return graphs inline (default true); false requires dumpToFile=true"),
    ...
    pathFilter: z.string().optional().describe("lint: content folder to scan recursively (default /Game)"),
    largeGraphThreshold: z.number().optional().describe("lint: node count above which a graph is flagged LargeGraph (default 300)"),
    code: z.string().optional().describe("author_logic/insert_logic: constrained C++-style logic block"),
    afterNodeId: z.string().optional().describe("insert_logic: guid or name of the node to splice logic after"),
