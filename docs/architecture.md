...
The server creates an `McpServer` instance (from `@modelcontextprotocol/sdk`), registers <!-- count:tools -->32<!-- /count --> category tools plus a `flow` tool, and communicates with the AI client over stdio.
...
29 C++ handler groups are registered in `BridgeServer.cpp`. Together with host-side MCP actions, the server exposes <!-- count:actions -->837+<!-- /count --> action names (some bridge names are aliases mapped onto a smaller number of canonical handlers):
