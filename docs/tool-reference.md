...
| `summarize_components` | Bounded component inspection for very large actors. Returns aggregate component and StaticMeshComponent visibility/render counts plus a capped nearest-components sample without serializing the full tree. Params: `actorLabel \\| internalName \\| actorPath, world? (editor\\|pie\\|auto), componentClass?, referenceActor?, usePlayerPawn?, maxDistance?, sampleLimit? (default 20, max 100)` |
